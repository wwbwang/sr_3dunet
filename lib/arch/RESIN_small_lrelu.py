...
class ResConv(nn.Module):
    def __init__(self, in_channels, out_channels, *, norm_type=None, dim=3):
        ...
        conv_layers.append(nn.LeakyReLU(0.2, True))
        ...
        if in_channels != out_channels:
            ...
            self.conv1x1 = None
        self.final_act = nn.LeakyReLU(0.2, True)
...
class RESIN_small_lrelu(nn.Module):
...
def get_model(args):
    model = RESIN_small_lrelu(in_channels=1, out_channels=1, 
...
if __name__ == '__main__':
    ...
    model = RESIN_small_lrelu(1, 1, [64,128,256], [64,128,256], norm_type=None)
    model.to(device)
