...
class RESIN_Loss(nn.Module):
    def __init__(self, model:RESIN_tiny, 
    ...
    def forward(self, real_A, model_out, it):
        if self.full_mip:
            self.real_A, self.fake_B, self.rec_A1, self.fake_B_T, self.rec_A2, self.ref_iso_mip= model_out
        else:
            self.real_A = real_A
            self.fake_B, self.rec_A1, self.fake_B_T, self.rec_A2 = model_out
            self.ref_iso_mip = get_slant_mip(self.real_A, angel=self.angel, iso_dim=self.iso_dim)
        ...
        if (it+1)%self.G_train_it == 0:
            self.set_requires_grad([self.model.D_MIP, self.model.D_RecA], False)
        ...
        if (it+1)%self.D_train_it == 0:
            self.set_requires_grad([self.model.D_MIP, self.model.D_RecA], True)
    ...
    def cal_loss_G(self):
        ...
        # cal MIP loss
        loss_aniso_mip = self.cal_GAN_loss(self.model.D_MIP, self.aniso_mip, True, is_disc=False)
        loss_halfIso_mip1 = self.cal_GAN_loss(self.model.D_MIP, self.halfIso_mip1, True, is_disc=False)
        loss_halfIso_mip2 = self.cal_GAN_loss(self.model.D_MIP, self.halfIso_mip2, True, is_disc=False)
        ...
        # cal Cube loss
        loss_rec_A = self.cal_GAN_loss(self.model.D_RecA, self.rec_A2, True, is_disc=False)
        self.loss_logger['loss_G/loss_rec_A'] = loss_rec_A.item()
    ...
    def cal_loss_D(self):
        ...
        # real Cube
        loss_real_pred_by_D_recA = self.cal_GAN_loss(self.model.D_RecA, self.real_A, True, is_disc=True)
        self.loss_logger['loss_D/loss_real_pred_by_D_recA'] = loss_real_pred_by_D_recA.item()

        # fake MIP
        loss_fake_pred_by_D_aniso = self.cal_GAN_loss(self.model.D_MIP, self.aniso_mip.detach(), False, is_disc=True)
        loss_fake_pred_by_D_iso1 = self.cal_GAN_loss(self.model.D_MIP, self.halfIso_mip1.detach(), False, is_disc=True)
        loss_fake_pred_by_D_iso2 = self.cal_GAN_loss(self.model.D_MIP, self.halfIso_mip2.detach(), False, is_disc=True)
        ...
        # fake Cube
        loss_fake_pred_by_D_recA = self.cal_GAN_loss(self.model.D_RecA, self.rec_A2.detach(), False, is_disc=True)
        self.loss_logger['loss_D/loss_fake_pred_by_D_recA'] = loss_fake_pred_by_D_recA.item()

        loss_D_real = loss_real_pred_by_D_aniso + loss_real_pred_by_D_iso1 + loss_real_pred_by_D_iso2 + \
                      loss_real_pred_by_D_recA
        loss_D_fake = loss_fake_pred_by_D_aniso + loss_fake_pred_by_D_iso1 + loss_fake_pred_by_D_iso2 + \
                      loss_fake_pred_by_D_recA
...
if __name__ == '__main__':
    ...
    model = RESIN_tiny(1, 1, [64,128,256], [64,128,256], norm_type=None, aniso_dim=-2, iso_dim=-1)
    model.to(device)
