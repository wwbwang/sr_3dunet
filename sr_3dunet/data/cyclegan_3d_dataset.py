...
class CycleGAN_3D_Dataset(data.Dataset):

    def __init__(self, opt):
        super(CycleGAN_3D_Dataset, self).__init__()
        self.opt = opt
        self.keys = []
    ...
    def __getitem__(self, index):
        ...
        img_name_1 = self.keys[index]
        img_name_2 = self.keys[(index + 1) % len(self.keys)]  # 获取下一个索引对应的图像
        
        img_1 = tifffile.imread(img_name_1)
        img_2 = tifffile.imread(img_name_2)
        
        # random crop
        img_1 = random_crop_3d(img_1, self.gt_size)
        img_2 = random_crop_3d(img_2, self.gt_size)
